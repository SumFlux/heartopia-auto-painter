...
from heartopia_app.domain.conversion import ConversionRequest, GRID_DIMENSIONS
...
class ConversionThread(QThread):
    ...
    def __init__(self, service: ConversionService, image_path: str, request: ConversionRequest):
        super().__init__()
    ...
    def run(self):
        try:
            result = self.service.convert_image(self.image_path, self.request)
...
class ConvertPage(QWidget):
    def __init__(self, state: WorkspaceState, conversion_service: ConversionService):
        super().__init__()
        self.state = state
    ...
    def _create_control_panel(self) -> QWidget:
        ...
        self.ratio_combo.currentTextChanged.connect(self._update_grid_info)
        ...
        self.level_combo.currentTextChanged.connect(self._update_grid_info)
    ...
    def _update_grid_info(self) -> None:
        ...
        dimensions = GRID_DIMENSIONS[ratio][level]
        width, height = dimensions
        self.grid_info_label.setText(f"网格尺寸: {width} × {height} 像素")

    def _on_enhance_toggled(self, state: int) -> None:
        enabled = bool(state)
    ...
    def _select_image(self) -> None:
        ...
        if file_path:
            ...
            self.state.source_image_path = Path(file_path)
    ...
    def _start_conversion(self) -> None:
        ...
        mode_hints = []
        if request.enhance:
            mode_hints.append(f"增强(饱和{request.saturation}/对比{request.contrast}/锐{request.sharpness})")
        if request.dither:
            mode_hints.append("抖动")
        hint = f"（{'+'.join(mode_hints)}）" if mode_hints else ""
        self.status_label.setText(f"正在转换{hint}...")

        self.thread = ConversionThread(self.conversion_service, self.image_path, request)
        self.thread.finished.connect(self._on_conversion_finished)
        self.thread.error.connect(self._on_conversion_error)
        self.thread.start()

    def _on_conversion_finished(self, result) -> None:
        self.state.pixel_data = result.pixel_data
    ...
    def _generate_preview(self, result) -> None:
        scale = max(1, min(10, 600 // max(result.grid_width, result.grid_height)))
        rgb_array = result.get_preview_image(scale=scale)
...
精细度: Level {stats['level']}
    ...
    def _export_json(self) -> None:
        ...
        if file_path:
            try:
                self.conversion_service.export_json(self.state.pixel_data, file_path)
    ...
    def _export_csv(self) -> None:
        ...
        if file_path:
            try:
                self.conversion_service.export_csv(self.state.pixel_data, file_path)
